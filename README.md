...
- Skill content is forked from MIT-licensed upstream repos: `obra/superpowers` (pro-execution, pro-pdd, pro-quality), `vercel-labs/skills` (find-skills in pro-core), `addyosmani/agent-skills` (interview-me + idea-refine in pro-pdd), `UditAkhourii/adhd` (adhd in pro-pdd), `Owl-Listener/designer-skills` (pro-design), `PaulRBerg/agent-skills` + `peterknezek/skills` (pro-testing; the `petrkindlmann/qa-skills` library is **bridged** via `/qa-engine`, not vendored), `Yoraexe/ceobe` + `Intense-Visions/harness-engineering` + `a5c-ai/babysitter` + `IvanTorresEdge/molcajete.ai` (pro-data), `gszhangwei/open-spdd` (pro-spdd), `garrytan/gstack` (pro-gstack), `vercel/next.js` (next-* skills in pro-nextjs). The `agent-browser` skill in pro-testing is forked from `vercel-labs/agent-browser` under Apache-2.0. Per-plugin `LICENSE` files document attribution. Per-file footers cite the upstream repo on each SKILL.md where applicable.
