...
| **Build** | `design-eng-bridge`◆ (animation judgment → emilkowalski/skills, via `/design-eng-skills`): strict motion review, codebase motion audit, animation vocabulary, Apple fluid-interface principles, curated library picks, prototype variants | `pro-design` |
...
- **`pro-design`** — frontend design skills: design tokens, accessibility audit (WCAG 2.2 POUR), motion system, typography scale (all from [Owl-Listener/designer-skills](https://github.com/Owl-Listener/designer-skills), MIT) + shadcn/ui composition with OKLCH theming, cva variants, compound components, Field form layout (from [agents-inc/skills](https://github.com/agents-inc/skills), MIT). Plus `design-eng-bridge` - a **bridge** (not vendored) to [`emilkowalski/skills`](https://github.com/emilkowalski/skills) (8 skills, MIT) installed on demand via the `/design-eng-skills` command (`npx skills add`). These cover animation *judgment* rather than implementation: `review-animations` (strict motion review against a craft bar), `improve-animations` (codebase-wide motion audit that emits executable plans), `find-animation-opportunities` (where motion is missing, and where to refuse it), `animation-vocabulary` (name an effect from a vague description), `apple-design` (gesture tracking, velocity handoff, momentum projection, rubber-banding, materials, optical type), `emil-design-eng` (the polish philosophy trunk), `pick-ui-library`, and `prototype`. Note that `review-animations`, `pick-ui-library`, and `prototype` are explicit-invoke-only upstream. Depends on `figma@claude-plugins-official` for Figma-MCP integration.
...
- Skill content is forked from MIT-licensed upstream repos: `obra/superpowers` (pro-core, pro-execution, pro-pdd, pro-quality), `addyosmani/agent-skills` (interview-me + idea-refine in pro-pdd), `UditAkhourii/adhd` (adhd in pro-pdd), `Owl-Listener/designer-skills` (pro-design), `PaulRBerg/agent-skills` + `peterknezek/skills` (pro-testing; the `petrkindlmann/qa-skills` library is **bridged** via `/qa-engine`, not vendored), `Yoraexe/ceobe` + `Intense-Visions/harness-engineering` + `a5c-ai/babysitter` + `IvanTorresEdge/molcajete.ai` (pro-data), `gszhangwei/open-spdd` (pro-spdd), `garrytan/gstack` (pro-gstack). The `agent-browser` skill in pro-testing is forked from `vercel-labs/agent-browser` under Apache-2.0. The `emilkowalski/skills` design-engineering library (MIT, Copyright (c) 2026 Emil Kowalski) is **bridged** via `/design-eng-skills`, not vendored. Per-plugin `LICENSE` files document attribution. Per-file footers cite the upstream repo on each SKILL.md where applicable.
