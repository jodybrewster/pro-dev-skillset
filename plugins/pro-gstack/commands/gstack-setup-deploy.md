...
Run the GStack `setup-deploy` workflow through the local `pro-gstack` adapter.

**Input**: Use the arguments after `/gstack-setup-deploy` as the workflow request, URL, file reference, plan, diff, or context.

**Adapter rules**:

- Read the matching adapter skill at `skills/gstack-setup-deploy/SKILL.md`.
- Use the vendored upstream source at `upstream/gstack/setup-deploy/SKILL.md` as reference material for the workflow intent, specialist role, review criteria, and output format.
- Do not expose or invoke the unprefixed upstream command name. This command is intentionally namespaced as `/gstack-setup-deploy`.
