...
Run the GStack `investigate` workflow through the local `pro-gstack` adapter.

**Input**: Use the arguments after `/gstack-investigate` as the workflow request, URL, file reference, plan, diff, or context.

**Adapter rules**:

- Read the matching adapter skill at `skills/gstack-investigate/SKILL.md`.
- Use the vendored upstream source at `upstream/gstack/investigate/SKILL.md` as reference material for the workflow intent, specialist role, review criteria, and output format.
- Do not expose or invoke the unprefixed upstream command name. This command is intentionally namespaced as `/gstack-investigate`.
