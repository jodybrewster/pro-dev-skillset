...
This skill is expensive. About 10 subagent calls, 30 to 90 seconds wall
clock, 5 to 10x a single answer. Do not pay that cost when a direct answer
is better. Run this gate before Phase 1.
...
2. Dispatch 5 **parallel, isolated** subagents — one per frame (Claude Code:
   the Agent/Task tool with `general-purpose`; Codex: a subagent or a fresh
   agent run). Each subagent gets only:
   - the problem P
   - any context the user provided
   - the chosen frame's vantage prompt
   - a system instruction that forbids evaluation

   The exact instruction to give each subagent:
...
3. **Critical invariant.** The subagent calls must be parallel and
   isolated. Do NOT serialize them. Do NOT pass one branch's output as
   context to another. Branches that see each other anchor each other and
   the whole method collapses to a wider single thought.
...
3. **Deepen the top 3.** Rank by weighted score (novelty 0.35 + viability
   0.40 + fit 0.25), exclude traps, take top 3. For each, dispatch one
   subagent call that produces:
...
3. **Converge.** A 2 to 4 idea shortlist. State why each is on the list.
   Mark the non-obvious-but-viable pick explicitly with a star. List traps
...
- **Skipping the isolation invariant.** If you simulate parallel branches
  ...
  have done a wider single thought. Give each branch a genuinely isolated
  subagent context. Use it.

## Calibration

- **How many ideas?** Scale to stakes. Quick "name this function" =
  3 frames x 4 ideas. "How should I position this product" = 5 frames x
  8 ideas. Default is 5 x 6 = 30.
...
5 diverge + 1 score + 1 cluster + 3 deepen ~= 10 subagent calls per run.
...
agentic coding harness or in batch.
...
loop inside Claude Code or Codex with no install required.
...
reference. The implementation choices made here (parallel isolated
subagent calls, mechanical generator/critic split, frame-based branching)
follow from that spec.

---

_Forked from [UditAkhourii/adhd](https://github.com/UditAkhourii/adhd) — MIT License. See original repository for full license text._
