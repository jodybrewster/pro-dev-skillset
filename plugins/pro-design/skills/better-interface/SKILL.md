...
description: "Cross-discipline interface review: the consolidation and verdict layer for the better-* family. Routes a screen, flow, feature, or whole repository to every available domain skill across accessibility, layout, typography, colour, writing, and UI craft, then consolidates one ranked, capped verdict. Any domain whose better-* skill is not installed is reported as Not reviewed rather than guessed. Use for a holistic review rather than a single domain, and as the hand-off target when interface-review hands up a change. A critique of a screenshot or rendered screen with no code goes to screen-critique, a change-scoped review starts at interface-review, and general code review is requesting-code-review. Triggers on better-interface, holistic interface review, review the whole screen."
---
...
## Sibling skills not vendored here

This file routes to other skills from the same collection (`better-accessibility`, `better-typography`, `better-colors`, `better-writing`, `better-ui`).
pro-design ships `interface-review`, `better-interface`, and `better-layout` from it; the rest are not vendored.
Until they are installed, `better-interface` reports those domains as `Not reviewed` instead of guessing.
Install any of them with `/design-skills install jakub <name>`, or all of them with `/design-skills install jakub`.

---

_Forked from [jakubkrehel/skills](https://github.com/jakubkrehel/skills) - MIT License. See original repository for full license text._
