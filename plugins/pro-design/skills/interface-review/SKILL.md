...
description: "Visual and UX quality pass over a UI change - the rendered interface a diff produces, compared against the interface before it. Use only when the ask explicitly names the UI, the design, the visuals, a component, a screen, animation, accessibility, or responsive behaviour. Reads both sides of the change, flags visual regressions and names the edit that caused them, and returns a scored, itemised list to iterate on. An unqualified request to review a pull request, review a diff, or review a branch carries no design framing and is ordinary code review: it belongs to requesting-code-review, not here. Judging one rendered screen with no change to compare is screen-critique."
---
...
This file routes to other skills from the same collection (`better-accessibility`, `better-typography`, `better-colors`, `better-writing`).
