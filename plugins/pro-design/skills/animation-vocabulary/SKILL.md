...
description: Reverse-lookup glossary that turns a vague description of desired motion (snappy, bouncy, smooth, feels heavy, the iOS rubber-band scroll) into the precise animation terms a model or designer can implement, meaning easing curves, durations, spring parameters, and choreography. Use before implementing when the motion ask is ambiguous, or when someone asks what an effect is called and wants the right word to prompt with. It names the effect rather than designing or building one, animate does the work and motion-system records the tokens.
---
