---
name: animate
description: Adds or refines animation on an already built site or app. Use when asked to animate something, add motion, or build a transition. Decides what needs movement and what does not, syncs motion with scroll and state, picks tool, properties, curve, duration, interruption and exit in that order, then writes the implementation. Ships small functional animations such as form submit, hover reveals, toasts, drawers, and tab indicators. The motion skill is the Framer Motion rulebook, motion-system defines tokens, and gsap or hyperframes cover marketing hero sequences and complex timelines.
---
...
This file names other skills from the same collection (`review-animations`, `improve-animations`, `find-animation-opportunities`, `animate-expo`, `pick-ui-library`).
They are not shipped in pro-design.
Install any of them with `/design-skills install emil <name>`; `screen-critique` covers the review case and `shadcn-ui-composition` covers library composition in the meantime.

---

_Forked from [emilkowalski/skills](https://github.com/emilkowalski/skills) - MIT License. See original repository for full license text._
